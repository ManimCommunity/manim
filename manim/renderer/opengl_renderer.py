...
class OpenGLRenderer:
    ...
    # topLeft flag should be set to True when using a GUI framework 
    # where the (0,0) is at the top left: e.g. PySide6
    def pixel_coords_to_space_coords(self, px, py, relative=False, topLeft=False):
        pixel_shape = self.get_pixel_shape()
        ...
        if relative:
            ...
            scale = fh / ph
            return fc + scale * np.array([(px - pw / 2), (-1 if topLeft else 1) * (py - ph / 2), 0])
