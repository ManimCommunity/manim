...
__all__ = ["ThreeDVMobject", "ParametricSurface", "Sphere", "Cube", "Prism", "Cone", "Arrow3D", "Cylinder", "Line3D"]
...
class Cone(ParametricSurface):
	def __init__(
		...
		**kwargs
		...
		ParametricSurface.__init__(self, 
			self.func,
			v_min = v_min,
			v_max = v_max,
			u_min = u_min,
...
class Arrow3D(VGroup):
	def __init__(self, start, end, height=0.5, base_radius=0.25, color=WHITE, **kwargs):
		VGroup.__init__(self, **kwargs)
		self.line = Line3D(start, end)
		self.line.set_color(color)

		self.cone = Cone(direction=self.direction, height=height, base_radius=base_radius, **kwargs)
		self.cone.shift(end)
...
class Line3D(Cylinder):
	def __init__(
		...
		**kwargs):
		...
		super().__init__(
			height=get_norm(self.direction),
			radius=width,
			direction=self.direction,
			**kwargs
			)
