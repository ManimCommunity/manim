    ...
    def set_color_by_t2c(self, t2c=None):
        ...
        for word, color in list(t2c.items()):
            for start, end in self.find_indexes(word, self.original_text):
                start_shift = self.original_text[:start].count(" ")
                end_shift = self.original_text[:end].count(" ")
                self.chars[start - start_shift : end - end_shift].set_color(color)
    ...
    def set_color_by_t2g(self, t2g=None):
        ...
        for word, gradient in list(t2g.items()):
            for start, end in self.find_indexes(word, self.original_text):
                start_shift = self.original_text[:start].count(" ")
                end_shift = self.original_text[:end].count(" ")
                self.chars[start - start_shift : end - end_shift].set_color_by_gradient(
                    *gradient
                )
