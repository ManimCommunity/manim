...
from .opengl_geometry import OpenGLDot, OpenGLLine, OpenGLLabeledDot
from .svg.opengl_tex_mobject import OpenGLMathTex
...
class OpenGLGraph(OpenGLVMobject):
    ...
    def __init__(
        ...
        layout_scale: float = 2,
        layout_config: Union[dict, None] = None,
        vertex_type: "Mobject" = OpenGLDot,
        vertex_config: Union[dict, None] = None,
        edge_type: "Mobject" = OpenGLLine,
        partitions: Union[List[List[Hashable]], None] = None,
        root_vertex: Union[Hashable, None] = None,
        edge_config: Union[dict, None] = None,
        offset_edges_by_radius: bool = False,
    ) -> None:
        OpenGLVMobject.__init__(self)
        ...
        if isinstance(labels, dict):
            ...
            if labels:
                self._labels = {
                    v: OpenGLMathTex(v, fill_color=label_fill_color) for v in vertices
                }
            else:
                self._labels = {}

        if self._labels and vertex_type is OpenGLDot:
            vertex_type = OpenGLLabeledDot
        ...
        if offset_edges_by_radius:
            for vertex in self.vertices.values():
                assert hasattr(
                    vertex, "radius"
                ), f"Passed offset_edges_by_radius, but {str(vertex)} has no radius attribute."

        self.edges = {}
        for (u, v) in edges:
            u_v_vector = normalize(self[v].get_center() - self[u].get_center())
            self.edges[(u, v)] = edge_type(
                self[u].get_center() + u_v_vector * self[u].radius,
                self[v].get_center() - u_v_vector * self[v].radius,
                z_index=-1,
        ...
        def update_edges(graph):
            for (u, v), edge in graph.edges.items():
                ...
                edge.put_start_and_end_on(
                    graph[u].get_center() + u_v_vector * self[u].radius,
                    graph[v].get_center() - u_v_vector * self[v].radius,
                )
    ...
    def from_networkx(nxgraph: nx.classes.graph.Graph, **kwargs) -> "Graph":
        ...
        **kwargs
        ...
        """
        return OpenGLGraph(list(nxgraph.nodes), list(nxgraph.edges), **kwargs)
