...
class CurvesAsSubmobjects(VGroup):
    ...
    def _throw_error_if_no_submobjects(self):
        if len(self.submobjects) == 0:
            ...
            raise Exception(
                f"Cannot call {cls_name}.{caller_name} for a {cls_name} with no submobjects"
            )
    ...
    def _get_submobjects_with_points(self):
        ...
        if len(submobjs_with_pts) == 0:
            ...
            cls_name = type(self).__name__
            raise Exception(
                f"Cannot call {cls_name}.{caller_name} for a {cls_name} whose submobjects have no points"
            )
