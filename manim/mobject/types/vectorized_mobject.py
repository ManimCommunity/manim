...
class VMobject(Mobject):
    ...
    def update_rgbas_array(
        ...
        """ Updates the given array: array_name, with the r,g,b,a values 
        corresponding to the given color and opacity/alpha value, 
        which is generated by the generate_rgbas_array method.
        
        
        Parameter
        ---------
            array_name: Depending upon what is to be changed, it could be either one of:

                        "stroke_rgbas": when stroke color/opacity is to be changed.
                        
                        "background_stroke_rgbas": when background stroke color/opacity is to be changed.
                        
                        "fill_rgbas": when fill color/opacity is to be changed. 
            
            color: The new color which is to be used in place of the existing color.

            opacity: The new opacity value which is to be used in place of the existing opacity value.
                    It's value can be from 0 to 1, including both values.

        Note: alpha is the technical name for opacity. Hence, the method's name has the acronym rgba in it.
        
        This method is generally not directly called by user. It is used internally. 
        ...
        current_rgbas = getattr(self, array_name)
        if len(current_rgbas) < len(rgbas):
            current_rgbas = stretch_array_to_length(current_rgbas, len(rgbas))
            setattr(self, array_name, current_rgbas)
        elif len(rgbas) < len(current_rgbas):
            rgbas = stretch_array_to_length(rgbas, len(current_rgbas))
        # Only update rgb if color was not None, and only
        # update alpha channel if opacity was passed in
        if color is not None:
            current_rgbas[:, :3] = rgbas[:, :3]
    ...
    def set_stroke(
        ...
        """ Sets the stroke_color/background_stroke_color or the opacity of those attributes, or width of those attributes of the :class:`VMobject`.

        Notes
        -----
            This method is intended to be called directly by the user.
            
        Parameters
        ----------
        color
            If background = False (it's the default value), then color represents the stroke_color of the :class:`VMobject`.
            If background = True, then color represents the background_stroke_color of the :class:`VMobject`.
        
        width
            it is the width of the stroke, that the user wants to set.
            If background = False, then width represents stroke_width.
            If background = True, then width represents background_stroke_width.

        opacity
            Opacity of either the stroke_color or the background_stroke_color.
        
        background
            If ``True``, the ``background_stroke_color`` and related attributes
            are modified instead of the foreground stroke attributes.
             
        family
            If ``True``, this method is applied to the submobjects recursively.
    ...
    def get_style(self, simple: bool = False) -> dict:
        """ Returns a dictionary comprising of the attributes which determine:            
            color(fill_color, stroke_color, background_stroke_color),
            opacity of those color attribute,            
            width(stroke_width, background_stroke_width),            
            sheen,            
            background of the :class:`VMobject`.

        Parameter
        ---------
            simple: If this is set to True, then only the very basic 5 attributes 
                    will be returned, viz.: stroke_color, stroke_opacity, stroke_width, fill_color and fill_opacity.
                    
                    When set to False(which is the default value), then the following 11 attributes
                    will be returned: stroke_color, stroke_opacity, stroke_width, fill_color, fill_opacity,
                    background_stroke_color, background_stroke_width, background_stroke_opacity, sheen_factor, 
                    sheen_direction and background_image 
        ...
        if simple:
            return_dict["fill_color"] = self.get_fill_color()
            return_dict["fill_opacity"] = self.get_fill_opacity()
            return_dict["stroke_color"] = self.get_stroke_color()
        else:
            return_dict["fill_color"] = self.get_fill_colors()
            return_dict["fill_opacity"] = self.get_fill_opacities()
            return_dict["stroke_color"] = self.get_stroke_colors()
            return_dict["background_stroke_color"] = self.get_stroke_colors(background=True)
            return_dict["background_stroke_width"] = self.get_stroke_width(background=True)
            return_dict["background_stroke_opacity"] = self.get_stroke_opacity(background=True)
            return_dict["sheen_factor"] = self.get_sheen_factor()
            return_dict["sheen_direction"] = self.get_sheen_direction()
            return_dict["background_image"] = self.get_background_image()
    ...
    def set_color(self, color: ParsableManimColor, family: bool = True) -> Self:
        """ Sets the fill_color and the stroke_color of the VMobject.
        Note that this does not affect the background stroke color.

        Parameters
        ----------
            color
                The color to apply to both the fill and the stroke.
            family
                If True, the color is applied recursively to all submobjects.
    ...
    def set_opacity(self, opacity: float, family: bool = True) -> Self:
        """ Sets the opacity of fill_color, stroke_color and the background_stroke_color 
        of the VMobject and all its submobjects by default.        
         Parameters
        ----------
            opacity
                The opacity value to apply, where 0.0 is fully transparent and 1.0 is fully opaque.
            
            family
                If True(default value), the opacity is applied recursively to all submobjects.
    ...
    def scale(
        ...
        *,
        ...
        scale_stroke_width
            Boolean determining if the VMobject's stroke_width is scaled when the object is scaled.
            If enabled, and object with 2px outline is scaled by a factor of .5, 
            it will have an outline of 1px.
        kwargs
    ...
    def set_sheen(
        ...
        """Applies a color gradient in the given direction.
    ...
    def get_gradient_start_and_end_points(self) -> tuple[Point3D, Point3D]:
        if self.shade_in_3d:
            ...
            c = self.get_center()
            new_unit_vectors = np.array(
                [self.get_edge_center(vect) - c for vect in [RIGHT, UP, OUT]],
            ).transpose()
            offset = np.dot(new_unit_vectors, direction)
            return (c - offset, c + offset)

    def color_using_background_image(self, background_image: Image | str) -> Self:
        self.background_image: Image | str = background_image  # str is the path of the image
    ...
    def set_anchors_and_handles(
        ...
        """Sets self.points by interleaving four arrays of points — anchors1, handles1,
        handles2, and anchors2 — into a single flat array, where every group of 4
        consecutive rows represents one cubic Bezier curve.

        Each cubic Bezier curve i is defined by exactly 4 points:
            - anchors1[i] : start anchor point of the i-th curve
            - handles1[i] : first  control handle point of the i-th curve
            - handles2[i] : second control handle point of the i-th curve
            - anchors2[i] : end anchor point of the i-th curve

        The resulting self.points is a flat array of shape
        (4 * length of any of the given array, 3), where every group of 4 consecutive rows
        represents one complete cubic Bezier curve:

            self.points = [
                anchors1[0], handles1[0], handles2[0], anchors2[0],  # curve 0
                anchors1[1], handles1[1], handles2[1], anchors2[1],  # curve 1
                ...
                anchors1[n], handles1[n], handles2[n], anchors2[n],  # curve n
            ]

        Example (2 curves, dim=3):
            anchors1 = [[0,0,0], [4,0,0]]
            handles1 = [[1,2,0], [5,2,0]]
            handles2 = [[2,2,0], [6,2,0]]
            anchors2 = [[3,0,0], [7,0,0]]

            self.points = [
                [0,0,0],  ← anchors1[0]  ┐
                [1,2,0],  ← handles1[0]  │ curve 0
                [2,2,0],  ← handles2[0]  │
                [3,0,0],  ← anchors2[0]  ┘
                [4,0,0],  ← anchors1[1]  ┐
                [5,2,0],  ← handles1[1]  │ curve 1
                [6,2,0],  ← handles2[1]  │
                [7,0,0],  ← anchors2[1]  ┘
            ]
        ...
        Note
        ----
            The four hardcoded parameters (anchors1, handles1, handles2, anchors2)
            are required as the number of points per cubic curve is fixed at 4 for a cubic bezier curve,
            which is what manimce uses for drawing VMobjects, which as a side effect also implies that
            this method cannot be generalized to non-cubic Bezier curves without
            a redesign.

        Parameters
        ----------
        anchors1 : Point3DLike_Array
            Array of shape (N, 3) containing the start anchor of each curve.
        handles1 : Point3DLike_Array
            Array of shape (N, 3) containing the first control handle of each curve.
        handles2 : Point3DLike_Array
            Array of shape (N, 3) containing the second control handle of each curve.
        anchors2 : Point3DLike_Array
            Array of shape (N, 3) containing the end anchor of each curve.
        ...
        if(len(new_points) % 4 != 0):
            if not (len(self.points) %4 == 1 and len(new_points) %4 == 3):
                if len(self.points) %4 == 1:
                    raise ValueError("Your existing shape has an incomplete curve waiting to be finished. "
                    ...
                    raise ValueError("This method requires valid number of points, each having the shape (3,) to work. "
    ...
    def start_new_path(self, point: Point3DLike) -> Self:
        """Append a point which is a 1D numpy array of shape (3,) to the :attr:`VMobject.points`, 
        which will be the beginning of a new Bézier curve in the path given by the points.
        If there's an unfinished curve at the end of :attr:`VMobject.points`,
        it completes it by appending the last Bézier curve's start anchor as many
        times as needed.

        Parameters
        ----------
        point
            A 1D numpy array of shape (3,) to append to :attr:`VMobject.points`.
        ...
        if n_points % nppc != 0:
            # close the open path by appending the last bezier curve's start anchor point sufficiently often.
    ...
    def add_cubic_bezier_curve(
        ...
        """Appends a single cubic Bezier curve to the VMobject.

        Parameters
        ----------
        anchor1
            The start anchor of the curve, a 1D numpy array of shape (3,).
        handle1
            The first control handle of the curve, a 1D numpy array of shape (3,).
        handle2
            The second control handle of the curve, a 1D numpy array of shape (3,).
        anchor2
            The end anchor of the curve, a 1D numpy array of shape (3,).

        Returns
        -------
        :class:`VMobject`
            The VMobject itself, after appending the curve, to allow method chaining..
        """
        if len(self.points) %4 != 0:
            raise ValueError("Cannot add a new cubic Bezier curve because the existing "
                            "self.points has an incomplete curve. "
                            f"len(self.points) = {len(self.points)}, which is not a multiple of 4.")
        self.append_points([anchor1, handle1, handle2, anchor2])
        return self

    def add_cubic_bezier_curves(self, curves) -> Self:
        """Adds either a single or multiple cubic bezier curve to the VMobject.

        Parameters
        ----------
        curves
            A 3D numpy array of shape(N,4,3), where N is the number of bezier curves to be added. 
            Each cubic bezier curve has 4 points; each point is a 1D numpy array of shape (3,) 
            representing the x, y, z coordinates of the point.
            Each set of 4 such points represents (start anchor, handle1, handle2, end anchor).
        
        Returns
        -------
        The VMobject itself to allow method chaining.
        """
        self.append_points(curves.reshape(-1,3))
        return self
    ...
    def add_cubic_bezier_curve_to(
        ...
        """Adds a cubic bezier curve to the VMobject.

        NOTE : the first anchor is not a parameter, as by default the end anchor point 
        of the last cubic bezier curve of the VMobject acts as the start anchor point of
        the cubic bezier curve being added.

        Parameters
        ----------
        handle1
            first handle of the cubic bezier curve.
        handle2
            second handle of the cubic bezier curve.
        end_anchor
            end anchor of the cubic bezier curve.
        ...
        new_points = [handle1, handle2, end_anchor]
    ...
    def add_quadratic_bezier_curve_to(
        ...
        """Add Quadratic bezier curve to the VMobject.
        ...
        self.add_cubic_bezier_curve_to(
            1 / 3 * self.get_last_point() + 2 / 3 * handle,
    ...
    def add_line_to(self, point: Point3DLike) -> Self:
        """Adds a straight line from the last point of VMobject to the given point.
    ...
    def add_smooth_curve_to(self, new_point: Point3DLike, handle2: Point3DLike | None = None) -> Self:
        """Creates a smooth curve from given points and adds it to the VMobject.

        Parameters
        ----------
        new_point
            Its the end anchor point of the new cubic Bezier curve being added.
            Its a 1D numpy array of shape (3,) or a tuple of 3 floats.
            
        handle2
            It is a 1D numpy array of shape (3,) or a tuple of 3 floats.
            If None, it is computed automatically from the last tangent.
            
        Note
        ----
        If last_tangent is zero, a ValueError is raised.
        If handle2 is None and new_point equals the last anchor point, a ValueError is raised.
        ...
        if self.has_new_path_started():
            ...
            if self.consider_points_equals(last_tangent, (0.0, 0.0, 0.0)):
                raise ValueError("last_tangent is zero. Can't guarantee a smooth cubic Bezier curve.")
            handle1 = last_a2 + last_tangent
            if handle2 is None:
                to_anchor_vect = new_point - last_a2
                if self.consider_points_equals(to_anchor_vect, (0.0, 0.0, 0.0)):
                    raise ValueError("new_point is identical to the last anchor point. "
                                    "Cannot create a meaningful curve to the same point.")
                new_tangent = rotate_vector(last_tangent, PI, axis=to_anchor_vect)
                handle2 = new_point - new_tangent
            self.append_points([last_a2, handle1, handle2, new_point])
        return self
    ...
    def consider_points_equals(self, p0: Point3DLike, p1: Point3DLike) -> bool:
        """Determines if two points are close enough to be considered equal.

        This uses the algorithm from np.isclose(), but expanded here for the
        3D point case. NumPy is overkill for such a small question.
        Parameters
        ----------
        p0
            first point
        ...
        -------
        bool
            when two points are considered close enough.
        """
        ...
        if abs(p0[1] - p1[1]) > atol + rtol * abs(p1[1]):
            return False
        return abs(p0[2] - p1[2]) <= atol + rtol * abs(p1[2])

    def consider_points_equals_2d(self, p0: Point2DLike, p1: Point2DLike) -> bool:
        """Deprecated: use the method consider_points_equals instead."""
        import warnings
        warnings.warn(
            "consider_points_equals_2d is deprecated. "
            "Use consider_points_equals instead.",
            DeprecationWarning,
            stacklevel=2,
        )
