...
class VMobject(Mobject):
    ...
    def get_style(self, simple: bool = False) -> dict:
        ...
        if simple:
            ...
            return_dict["background_stroke_color"] = self.get_stroke_colors(
                background=True
            )
            return_dict["background_stroke_width"] = self.get_stroke_width(
                background=True
            )
            return_dict["background_stroke_opacity"] = self.get_stroke_opacity(
                background=True
            )
    ...
    def color_using_background_image(self, background_image: Image | str) -> Self:
        self.background_image: Image | str = (
            background_image  # str is the path of the image
        )
    ...
    def add_cubic_bezier_curve(
        ...
        if len(self.points) % 4 != 0:
            raise ValueError(
                "Cannot add a new cubic Bezier curve because the existing "
    ...
    def add_smooth_curve_to(
        self, new_point: Point3DLike, handle2: Point3DLike | None = None
    ) -> Self:
        ...
        if self.has_new_path_started():
            ...
            if self.consider_points_equals(last_tangent, (0.0, 0.0, 0.0)):
                raise ValueError(
                    "last_tangent is zero. Can't guarantee a smooth cubic Bezier curve."
                )
            ...
            if handle2 is None:
                ...
                if self.consider_points_equals(to_anchor_vect, (0.0, 0.0, 0.0)):
                    raise ValueError(
                        "new_point is identical to the last anchor point. "
