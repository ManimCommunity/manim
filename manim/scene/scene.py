...
class Scene:
    ...
    def __init__(
        ...
        self.relative_speed_mode = False
        self.queue = Queue()
    ...
    def scene_speed_changing(
        ...
        relative_speed_mode=False,
        retain_final_speed=False,
    ):
        """
        ...
        relative_speed_mode : bool, optional
            If false, the given speeds are strictly respected but the
            duration of animation is scaled accordingly.
            If true, ``run_time`` is strictly respected but speed changes are
            only applied relative to each other in the given animation.
        retain_final_speed : bool, optional
            Speed of scene, ``self.speed``, will remain changed to final speed
            on exit, if true.
        """
        self.animation_speedinfo = animation_speedinfo
        self.relative_speed_mode = relative_speed_mode
        yield
        self.animation_speedinfo = None
    ...
    def get_time_progression(
        ...
        if self.renderer.skip_animations and not override_skip_animations:
            ...
            if animation_speedinfo is not None:
                ...
                if self.relative_speed_mode:
                    # Dry run for the speed change loop below to calculate the
                    # number of frames generated by it, so that time stamps
                    # can be rescaled to strictly follow the run_time
                    speed_fac = 0
                    for node, speed in sorted(animation_speedinfo.items()):
                        ...
                        speed_fac += func_unity_at * adj_time / self.speed
                        self.speed = speed
                        prev_node = node
                    prev_node = 0
                    self.speed = init_speed * speed_fac
    ...
    def play(self, *args, animation_speedinfo=None, relative_speed_mode=None, **kwargs):
        if animation_speedinfo is not None:
            self.animation_speedinfo = animation_speedinfo
        if relative_speed_mode is not None:
            self.relative_speed_mode = relative_speed_mode
        self.renderer.play(self, *args, **kwargs)

    def wait(self, duration=DEFAULT_WAIT_TIME, stop_condition=None, **kwargs):
        self.play(Wait(run_time=duration, stop_condition=stop_condition), **kwargs)

    def wait_until(self, stop_condition, max_time=60, **kwargs):
        """
        ...
        """
        self.wait(max_time, stop_condition=stop_condition, **kwargs)
