...
def project_setup():
    ...
    config_data["config.partial_movie_dir"] = prompt_dir_config_attribute("partial_movie_dir")
    ...
    if verbosity_level.strip() == "":
        ...
        if not verbosity_level.upper() in verbosity_level_options:
            raise NameError(f"{verbosity_level} is not one of {verbosity_level_options}")
        else:
    ...
    if media_width_input.strip() == "":
        ...
        if not is_percent(media_width_input):
            raise ValueError(f"Expected percentage input including '%' but received type({media_width_input}) = {type(media_width_input)}")
        ...
        if is_number(_media_width_input):
            ...
            if float(_media_width_input) > 0 and float(_media_width_input) <= 100:
                ...
                raise ValueError(f"{media_width_input} is not within bounds of [0, 100]%")
        else:
            raise TypeError(f"{media_width_input} is not a number representing a percentage")
    ...
    config_data["config.frame_width"] = prompt_number_config_attribute("frame_width", float(0), float("inf")) # use -1 for infinity

    # Prompt user for frame height (float) # default config.frame_height = 8.0
    config_data["config.frame_height"] = prompt_number_config_attribute("frame_height", float(0), float("inf")) # use -1 for infinity
    ...
    config_data["config.pixel_width"] = prompt_number_config_attribute("pixel_width", 0, float("inf")) # use -1 for infinity
    
    # Prompt user for pixel height (int) # default config.pixel_height = 1080
    config_data["config.pixel_heights"] = prompt_number_config_attribute("pixel_height", 0, float("inf")) # use -1 for inifinity

    # Prompt user for frame rate (float) (0.0 fps - 60.0fps)
    config_data["config.frame_rate"] = prompt_number_config_attribute("frame_rate", float(0), float(60))
    ...
    if import_manimpango.lower() in false_types:
        ...
        print("For a comprehensive list of available fonts invoke `manimpango.list_fonts()`")
        config_data["manimpango"] = True
    else:
        raise NameError(f"Expected {true_types} or {false_types} but received type({import_manimpango}) = {type(import_manimpango)}")    
    ...
    if save_JSON.lower() in false_types:
        ...
        raise NameError(f"Expected {true_types} or {false_types} but received type({save_JSON}) = {type(save_JSON)}")    
...
def validate_folder_name(projectName):
    ...
    if not re.search("^[a-zA-Z0-9\s\/_-]*$", projectName):
        raise ValueError(f"Folder names [{projectName}] may consist only of alphanumerics, whitespace, hyphens, underscore & forwardslash characters")
...
def prompt_bool_config_attribute(attribute):
    ...
    if attribute_value.strip() == "":
        ...
        raise TypeError(f"Expected boolean type, but type({attribute}) = {type(attribute_value)}")
...
def prompt_background_color_config_attribute():  
    ...
    if backgroundColor.strip() == "":
        ...
        if not backgroundColor.upper() in vars(manim_colors):
            ...
            colors = [x for x in colors if x != "config"] # exclude config global variable from list
            print(colors)
            print("@ https://docs.manim.community/en/stable/reference/manim.utils.color.manim_colors.html")
            raise NameError(f"{backgroundColor} is not a valid color")
        else:
            config.background_color = backgroundColor # accept user input as config.background_color
...
def prompt_number_config_attribute(attribute, min, max):
    ...
    if numberInput.strip() == "":
        ...
        raise TypeError(f"Expected int or float input, but received type({numberInput}) = {type(numberInput)}")
    return config[f"{attribute}"]
