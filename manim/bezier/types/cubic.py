...
class ManimCubicBezier(ManimBezier):
    ...
    def bezier_remap(
        ...
        ----------
        quads
            The quads of the quadratic bezier curves to be remapped shape(n, 3, 3)
        ...
        Returns
        -------
            The new quads for the quadratic bezier curves.
        """
        quads = np.asarray(quads)
        difference = new_number_of_curves - quads.shape[0]
        if difference <= 0:
            return quads
        new_quads = np.zeros((new_number_of_curves, 3, 3))
        idx = 0
        for quad in quads:
            if difference > 0:
                tmp_noc = int(np.ceil(difference / quads.shape[0])) + 1
                tmp = cls.subdivide_bezier(quad, tmp_noc).reshape(-1, 3, 3)
                for i in range(tmp_noc):
                    new_quads[idx + i] = tmp[i]
        ...
        difference = new_number_of_curves - len(quads)
        if difference <= 0:
            return quads
        new_quads = []
        for quad in quads:
            if difference > 0:
                tmp_noc = int(np.ceil(difference / len(quads))) + 1
                tmp = subdivide_quadratic_bezier(quad, tmp_noc).reshape(-1, 3, 3)
                for i in range(tmp_noc):
                    new_quads.append(tmp[i])
                difference -= tmp_noc - 1
            else:
