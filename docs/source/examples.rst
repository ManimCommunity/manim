...
.. manim:: BooleanOperations
    ...
    class BooleanOperations(Scene):
        def construct(self):
            ...
            self.play(i.animate.scale(0.25))
            self.play(i.animate.move_to(RIGHT * 5 + UP * 2.5))
            intersection_text = Text("Intersection", font_size=23).next_to(i, UP)
            self.play(FadeIn(intersection_text))

            u = Union(ellipse1, ellipse2, color=ORANGE, fill_opacity=0.5)
            self.play(u.animate.scale(0.3))
            union_text = Text("Union", font_size=23)
            self.play(u.animate.next_to(i, DOWN, buff=union_text.height * 3))
            union_text.next_to(u, UP)
            self.play(FadeIn(union_text))

            e = Exclusion(ellipse1, ellipse2, color=YELLOW, fill_opacity=0.5)
            self.play(e.animate.scale(0.3))
            exclusion_text = Text("Exclusion", font_size=23)
            self.play(e.animate.next_to(u, DOWN, buff=exclusion_text.height * 3.5))
            exclusion_text.next_to(e, UP)
            self.play(FadeIn(exclusion_text))

            d = Difference(ellipse1, ellipse2, color=PINK, fill_opacity=0.5)
            self.play(d.animate.scale(0.3))
            difference_text = Text("Difference", font_size=23)
            self.play(d.animate.next_to(u, LEFT, buff=difference_text.height * 3.5))
