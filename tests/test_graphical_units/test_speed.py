...
def test_SpeedModifier(scene):
    ...
    scene.play(
        ChangeSpeed(
            AnimationGroup(
                a.animate(run_time=1, rate_func=linear).shift(RIGHT * 4),
                b.animate(run_time=1, rate_func=rush_from).shift(RIGHT * 4),
            ),
    ...
    scene.play(
        ChangeSpeed(
            AnimationGroup(
                a.animate(run_time=1, rate_func=linear).shift(LEFT * 4),
                b.animate(run_time=1, rate_func=rush_into).shift(LEFT * 4),
            ),
            speedinfo={0.3: 1, 0.4: 0.1, 0.6: 0.1, 1: 1},
