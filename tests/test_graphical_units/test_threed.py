...
def test_Z_Direction(scene):
    ...
    surface_plane.set_fill_by_value(
        axis="z", axes=axes, colors=[(RED, -0.4), (YELLOW, 0), (GREEN, 0.4)]
    )
...
def test_Y_Direction(scene):
    ...
    surface_plane.set_style(fill_opacity=1)
    surface_plane.set_fill_by_value(
        axis="y", axes=axes, colors=[(RED, -0.4), (YELLOW, 0), (GREEN, 0.4)]
    )
    scene.add(axes, surface_plane)


@frames_comparison(base_scene=ThreeDScene)
def test_X_Direction(scene):
    resolution_fa = 42
    scene.set_camera_orientation(phi=75 * DEGREES, theta=-120 * DEGREES)
    axes = ThreeDAxes(x_range=(0, 5, 1), y_range=(0, 5, 1), z_range=(-1, 1, 0.5))
    ...
    surface_plane.set_style(fill_opacity=1)
    surface_plane.set_fill_by_value(
        axis="x", axes=axes, colors=[(RED, -0.4), (YELLOW, 0), (GREEN, 0.4)]
    )
    scene.add(axes, surface_plane)
