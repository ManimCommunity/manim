...
def frames_comparison(
    ...
    **custom_config,
    ...
    """Compares the frames generated by the test with control frames specified with the test_name and module_name.
    ...
    .. warning::
        ...
        If the scene has a moving animation, then the test must have last_frame = False.
    """
...
def _get_control_data_path(
    ...
    if not setting_test and not path.is_file():
        raise Exception(
            f"The control frame for the test {test_name} cannot be found in {path.parent}. Make sure you generated the control frames first."
        )
    return path
