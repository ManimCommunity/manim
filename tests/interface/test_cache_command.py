...
@pytest.mark.parametrize("override_media_dir", [False, True])
def test_cache_clear_applies_config_file_and_path_overrides(
    tmp_path,
    override_media_dir,
):
    ...
    selected_media_dir = (
        tmp_path / "overridden-media" if override_media_dir else configured_media_dir
    )
    ...
    options = ["--config-file", str(config_file)]
    if override_media_dir:
        options.extend(["--media-dir", str(selected_media_dir)])
    result = CliRunner().invoke(
        main,
        [
            "cache",
